...
namespace UPTrace
{
	...
	public class UPTraceApi
	{
		 ...
		 * type：第三方sdk的类型标识，如'facebook', 'huawei'。
		 * playerId：玩家ID。
		 * token：登录成功后，第三方sdk返回的可用于服务端签名检验的参数
		 */
		public static void commonLogin(string type, string playerId, string token) {
			if (null == polyCall) {
				instanceOfCall ();
			}
			polyCall.commonLogin (type, playerId, token);
		}
		...
		public static void logLogin(string loginType, string playerId, string loginToken, string ggid, Dictionary<string, string> extension) {
            ...
            polyCall.logLogin (loginType, playerId, loginToken, ggid, extensionjson);
		}
